...
since: stable
keywords: [draft, book, reel, memory, hypothesis, status, ratify, open_question, dead_end]
related: [mos_draft_view, mos_book_query, mos_reel_get, mos_book_ratify, mos_book_open_question, mos_book_dead_end]
status: stable
---
...
| **L1 Draft** | process graph: hypotheses, plans, evidence, support edges | every EACN role | `branches/main/draft/draft.json` |
| **L2 Book** | durable compiled knowledge, citation-shaped | Ethics only | `branches/main/book/sources/*.md` |

## Top tools

```bash
lookup.py --id mos_draft_view          # FIRST call after wake (no-arg orient)
...
- `mos_draft_view` (no-arg) BEFORE `mos_await_events`. Its `orientation.pending_plans`
  surfaces `pending_plan` nodes left by your previous self before context reset.
...
## Ethics contract (L1 Draft)

Ethics' primary curation mode is `mos_draft_annotate` (update another role's
node) and `mos_draft_append(edges=[...])` (connect evidence, decisions, and
claim relationships across roles).
...
ANTI-PATTERN: do not mirror another role's node as a new Ethics-authored
claim. Use `mos_draft_annotate` to update the existing node's
`support_status` / `evidence_tag` / `confidence`.
...
| Layer | Expert self | Peer role | Ethics | Gru |
|---|---|---|---|---|
| **Reel-Index (L0)** | R own | — | R cross-role | R cross-role |
| **Draft (L1)** | RW own nodes | R all nodes | R + edge/status curation | R |
| **Book (L2)** | R | R | RW + ratify (`mos_book_ratify`) | R |

### Prose clarifications

- **Reel-Index reads beyond self** are restricted to Ethics and Gru. Peer Experts cannot read each other's Reel entries.
- **Draft** is fully readable team-wide. Any EACN-visible role appends nodes and edges for its own work via `mos_draft_append`. Ethics curates support status, evidence tags, and cross-role edges via `mos_draft_annotate` and `mos_draft_append`.
- **Book** has one write authority: only Ethics ingests pages and saves syntheses. Ethics also gates knowledge promotion via `mos_book_ratify` (Stream 3). All other roles read via `mos_book_query`.
