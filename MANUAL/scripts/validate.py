...
Runs three checks:
  ...
  2. Every tools/<name>.md page matches a real tool.
...
def main() -> int:
    ...
    if EACN3_TS.exists():
        mcp_files["mcp-servers/eacn3/plugin/index.ts"] = EACN3_TS.read_text(
            encoding="utf-8"
        ).splitlines()
    ...
    # 4. verbatim-description regression
    # Auto-generated pages should use a one-line source pointer. Any page
    # that re-ingests a verbatim plugin description re-introduces silent
    # drift that source-line checks cannot catch.
    for _pid, _fm, p in pages:
        body = p.read_text(encoding="utf-8")
        # Marker emitted by gen_eacn3_stubs.py for non-curated stubs.
        ...
        if "## Full description (from EACN3 plugin)" in body:
            errors.append(
                ...
                "'## Full description (from EACN3 plugin)' header. No page may re-ingest a "
