...
`_maybe_rebuild_corpus_graph` in `noter_wait.py` returns `{"rebuilt": False, "reason": "graphify venv not installed"}`. All roles still function — they just can't query the corpus graph. The wiki layer (Layer 2) works independently.
