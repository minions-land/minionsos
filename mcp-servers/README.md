...
All four are wired into the workspace via the top-level `.mcp.json` (Claude Code, generated by `minions/tools/_gen_mcp_json.py`) and `.codex/config.toml` (Codex). Adding a new MCP requires editing both generators in addition to dropping the implementation here (or, for a Python-package-coupled server, inside `minions/`).
