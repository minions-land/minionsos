...
| `graphify` | `mcp-servers/graphify/` (isolated `.venv` + `launcher.sh`) | Python (graphify lib) | `bash mcp-servers/graphify/launcher.sh` | **Optional** per-role graph analysis (prose): `query_graph`, `get_node`, `get_neighbors`, `get_community`, `god_nodes`, `graph_stats`, `shortest_path` |
| `codegraph` | `mcp-servers/codegraph/` (isolated `node_modules/` + `launcher.sh`) | Node / TypeScript (`@colbymchenry/codegraph`) | `bash mcp-servers/codegraph/launcher.sh` | **Optional** per-role code intelligence: light surface — `codegraph_search`, `codegraph_callers`, `codegraph_callees`, `codegraph_impact`, `codegraph_node`, `codegraph_files`, `codegraph_status`; heavy surface (Coder + Expert only at server-side authz) — `codegraph_context`, `codegraph_explore` |
