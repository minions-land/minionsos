...
All three are wired into the workspace via the top-level `.mcp.json` (Claude Code, generated by `minions/tools/_gen_mcp_json.py`). Adding a new MCP requires editing the generator in addition to dropping the implementation here (or, for a Python-package-coupled server, inside `minions/`).
...
# Node MCP — install.sh handles it, but for individual rebuild:
