...
/** Text color variant for inline status labels */
export function statusTextColor(s: TaskStatus): string {
  const map: Record<TaskStatus, string> = {
    unclaimed:          "text-amber-600",
    bidding:            "text-blue-600",
    awaiting_retrieval: "text-indigo-600",
    completed:          "text-emerald-600",
    no_one_able:        "text-red-500",
  };
  return map[s] ?? "text-stone-500";
}

/** Badge bg+text classes for task status pills */
export function statusBadge(s: TaskStatus): string {
  const map: Record<TaskStatus, string> = {
    unclaimed:          "bg-amber-100 text-amber-700",
    bidding:            "bg-blue-100 text-blue-700",
    awaiting_retrieval: "bg-indigo-100 text-indigo-700",
    completed:          "bg-emerald-100 text-emerald-700",
    no_one_able:        "bg-red-100 text-red-600",
  };
...
export function logColor(fn: string): string {
  ...
  if (fn.includes("submit_bid"))     return "text-blue-600";
