        ...
        for project in projects:
            for role in project.active_roles:
                ...
                if not events:
                    continue
                new_events = [
                    e for e in events if self._dedup.is_new(project.port, role.name, _event_id(e))
                ]
                if not new_events:
                    continue
                logger.info(
                ...
                if status == "veto":
                    ...
                    # Do NOT dispatch; re-deliver events next tick by
                    # forgetting them from the dedup cache.
                    for e in new_events:
                        self._dedup._seen.pop((project.port, role.name, _event_id(e)), None)
                    continue
                else:
