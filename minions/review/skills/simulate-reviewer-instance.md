...
4. **Spawn aspect subagents.** Each gets: current submission materials only, its aspect instructions, its stance excerpt, read-only / EACN-invisible / local-only boundary, output path, `templates/aspect-note.md` as the required format, and explicit permission to call the `codex` MCP tool (`sandbox="read-only"` for analysis, `sandbox="danger-full-access"` for code-trace work that must run scripts) for volume reading. Long manuscripts and code-heavy submissions should default to Codex delegation rather than line-by-line reading inside the subagent.
