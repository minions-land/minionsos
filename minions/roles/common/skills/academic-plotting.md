...
Plotting scripts at `branches/<expert>/paper/figures/gen_fig_<name>.py` read concrete data files from `branches/<expert>/exp/` or `branches/main/exp/exp-<id>/`. No hardcoded numbers. Re-run reproduces byte-identical output modulo font rendering.
