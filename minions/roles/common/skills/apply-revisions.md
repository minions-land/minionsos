...
2. **Build the checklist.** Read `branches/main/reviews/round-<n>/consolidated.md` required-revisions section and `branches/<expert>/paper/rebuttal/` response blocks. Extract one checklist entry per concern. Write the initial `revisions/round-<n>.md` with all entries `pending`.
