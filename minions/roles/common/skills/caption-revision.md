...
summary: Single-pass revise loop for figure / table captions — simulate one harsh review-pass critique, address the *caption text*. Never modify the figure itself. Forces Expert's single-pass output to absorb a critic before the caption ships.
layer: logical
tools:
version: 2
status: active
...
- Before submitting a figure for Expert or review feedback — never let an unreviewed caption out of Expert.
...
2. **Simulate one adversarial review-pass critic** by writing one sharp criticism. The voice should be skeptical, specific, and ungenerous. Pick the *single weakest claim* in the caption — over-claim, under-specification, missing N, missing statistic, drift between figure and caption text, hand-wave palette explanation. One sentence.
...
Ordered by frequency in formal review feedback:
...
- Does not multi-round. One critic, one fix. Multi-round caption iteration belongs to the formal review workflow, not this Expert-side pass.
