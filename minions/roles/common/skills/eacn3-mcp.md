...
- **Confusing EACN3 with the host runtime.** EACN3 only knows Servers, Agents, Domains, Credits, and Reputation. Projects, the Exploration DAG, role boundaries, workspace files — those concepts belong to MinionsOS, not the network. Don't expect the network to enforce host-side rules.
