...
- Gru or another role has completed feature intake and assigned implementation to Expert.
- A user request requires adding behavior rather than only debugging or review.
- A change that needs coordinated implementation — "coordinated" means it
  touches more than one Expert-owned module, or requires a handoff to another
  Expert, Ethics, or Gru.

If requirements are still ambiguous, do not start; ask through EACN first.

## Structure

Smallest viable implementation in Expert-owned paths. Seven phases: read the task, explore precedent, choose architecture, implement, verify, simplify, hand off. Each phase is one numbered step in Procedure.
...
- Expanding scope into unrelated scientific decisions, experiment execution,
  or review artifacts.
