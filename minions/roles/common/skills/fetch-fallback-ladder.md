...
summary: When a fetch tool fails (WebFetch domain refusal, HTTPS throttle, rate limit, blocked path), walk the ladder — paper-search MCP for arXiv/PubMed/etc, github-fetch for GitHub, codex MCP as last-resort generic fetch. Never stop at the first tool's failure.
layer: logical
tools: mos_search_arxiv, mos_search_papers_federated, mos_resolve_arxiv_ids, mos_read_arxiv_paper, mos_download_arxiv, codex, WebFetch, WebSearch, Bash
version: 1
status: active
...
|---|---|---|---|
| arXiv / PubMed / bioRxiv / medRxiv / Semantic Scholar paper or PDF | `mos_search_*` + `mos_read_*` / `mos_download_*` (see [[paper-literature-search]]) | `mos_search_papers_federated` for cross-source dedup | `codex` MCP with `sandbox=read-only` and an explicit fetch instruction |
| GitHub file / sub-tree / repo | `gh api` + `base64 -d` (see [[github-fetch]]) | `gh repo clone --depth=1` | `curl raw.githubusercontent.com` |
| arbitrary HTTPS (blog, vendor docs, dataset README) | `WebFetch` | `WebSearch` to find a mirror, then `WebFetch` the mirror | `codex` MCP — Codex has its own network stack and often clears domain refusals |
...
The ladder is **role-agnostic**. Every EACN-visible Role (Gru, Coder, Writer, Ethics, Expert) and Noter have `mos_search_*` and `mos_read_arxiv_paper` in their main whitelist; the codex MCP is in `_CODEX_BRIDGE_TOOLS`. Subagents inherit through their parent's main role's whitelist for paper-search and codex.
...
4. **If every rung fails, stop and report.** Name each rung tried, the exact failure mode, and what the next investigative step would be (check `gh auth status`, check codex availability via `mos_query_gpus`-style probe, ask the user for a mirror). Filing a `mos_issue_report(component="tool", severity="P2")` is appropriate when a rung that should have worked failed in a way that suggests scaffolding rot.
...
- **Skipping Rung 1 because Rung 3 looks easier.** Codex MCP is a generic last resort; using it for an arXiv paper wastes a paid roundtrip when `mos_read_arxiv_paper` is one call. Match the rung to the resource.
