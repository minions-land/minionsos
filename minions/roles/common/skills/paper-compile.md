...
1. **Verify prerequisites.** `which pdflatex latexmk bibtex`. Ensure `branches/<expert>/paper/main.tex`, `*.bib`, `sections/`, and `figures/` exist. Fail fast with a clear error if not.
2. **Clean build.** In `branches/<expert>/paper/`: `latexmk -C` then `latexmk -pdf -interaction=nonstopmode -halt-on-error main.tex 2>&1 | tee compile.log`.
