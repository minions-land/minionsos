...
summary: Guide for organizing workspace writes so the main branch stays coherent and extractable into a paper. Draft is the index; workspace holds the full content.
layer: structural
tools: mos_draft_append, mos_draft_annotate
version: 2
status: active
supersedes:
...
Every durable shared file should be findable by its Draft node. The Draft is the index; role branches and the shared worktree hold the full content. Connect them through `evidence_tag` references. A new agent joining the project should be able to navigate from Draft node → workspace file without guessing.

## When to invoke

- Before writing any durable file to the workspace — check if it fits the structure.
- During cognitive-checkpoint — ensure new Draft nodes have corresponding workspace files.
...
This is a recommendation, not a rigid schema. Adapt to your project's needs. The principle is navigability from Draft to content.

The Draft and workspace relate as index to content:

```
Draft node (index, one-liner)  ←→  Workspace file (full content)
...
1. **When writing a result:** create `experiments/E-xxx/results.md` and set the Draft node's `evidence_tag` to point there.
2. **When recording a dead end:** create `dead_ends/DEAD-xxx.md` with the full story (not just the one-liner in the Draft).
