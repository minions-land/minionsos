...
summary: Translate an accepted experiment request into queue-ready unit specs — decide unit boundaries, constraints, and estimates, then hand the spec to dispatch-runner. Never calls mos_exp_queue_submit.
layer: logical
...
3. **Adjust the GPU pool if policy changed.** If the user changed allowed GPUs, call `mos_exp_gpu_pool_set` now — the pool is the resource frame the queue runs against, and it must be correct before `dispatch-runner` submits. This is the only side effect this skill performs.
4. **Flag input staging.** If a unit's data is not already on its target, mark `needs_staging: true` with the source / destination paths. Do not call `mos_exp_put` here — `dispatch-runner` will run it before submission.
...
6. **Hand off to dispatch-runner.** Emit the plan: unit list, pool changes (if any), staging flags, artifact slots. Mark tool facts `[derived: allocate-resources plan @ <ts>]`. Do not call `mos_exp_queue_submit` — that is `dispatch-runner`'s job.
