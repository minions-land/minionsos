...
2. **Pick the archive location.** Reusable templates and lessons live under `artifacts/exp-templates/` or a clearly named note under `workspace/experiments/notes/`. Per-run bundles stay in `artifacts/exp-{id}/`; don't move them.
...
A short markdown file under `artifacts/exp-templates/` or `workspace/experiments/notes/`, with bidirectional pointer to the canonical `exp-{id}`. EACN announcement is one line.
