...
layer: logical
tools: mos_dag_query, mos_dag_append, mos_dag_annotate, eacn3_send_message
version: 2
status: active
supersedes:
...
Full audit producing two artifacts: `exploration/summary.md` (refreshed) and `artifacts/notes/dream-{date}.md` (new). Contradictions are escalated to Experts via EACN. The audit classifies problem nodes into four categories: stale, contradictions, orphans, and missing-evidence.
...
7. **Write dream report** to `artifacts/notes/dream-{YYYY-MM-DD}.md` summarizing actions taken and open issues.
...
- Exceeding the 120-line cap on `exploration/summary.md`.
- Unmarked claims — all written claims must carry `[derived: exploration/dag.json]`.
