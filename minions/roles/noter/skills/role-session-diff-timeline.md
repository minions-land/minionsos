...
summary: Open on every Noter wake before any other summary work — diff each role branch's archived session jsonl, summarize new ones into artifacts/notes/timeline.md; never drain EACN3 queues.
layer: logical
tools: eacn3_list_tasks, eacn3_get_task, eacn3_get_messages, eacn3_list_agents
version: 2
status: active
supersedes:
...
Every role wake leaves a jsonl transcript at `project_<port>/branches/<role>/.minionsos/sessions/<timestamp>-wake<NNN>.jsonl`. These are project-owned, git-tracked, and non-destructive to read. Noter scans them, summarises, appends to `artifacts/notes/timeline.md`.
...
State across wakes is held in a cursor at `artifacts/notes/.session-scan-cursor.json`:

```json
{"role_name": "last-processed-archive-filename.jsonl", ...}
```

Cursor writes are atomic (`.tmp` + rename). Noter writes only under `artifacts/notes/`.
...
3. **Read the scan cursor** at `artifacts/notes/.session-scan-cursor.json`.
...
6. **Append the block to `artifacts/notes/timeline.md`**, headed by the archive file's name so each block is traceable.
7. **Update the cursor file atomically** (write `.tmp`, then rename).
...
- **Don't write into role branches.** Branches belong to their owning roles; your write scope is `artifacts/notes/` only. Cursor file lives there too.
