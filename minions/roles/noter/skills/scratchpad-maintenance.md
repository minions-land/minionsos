---
slug: scratchpad-maintenance
summary: Keep the Scratchpad (L1) structurally healthy — connect dead ends to hypotheses, build same-topic chains, bridge topics through decisions, target edge density >1.0 edges/node.
layer: logical
tools: mos_scratchpad_query, mos_scratchpad_append, mos_scratchpad_annotate, mos_scratchpad_summary
version: 3
status: active
supersedes: dag-maintenance
references: full-dream, micro-dream
provenance: human
---

# Skill — Scratchpad Maintenance

A well-maintained Scratchpad has high edge density, meaningful communities, and no orphans. This skill keeps the graph structurally healthy so all roles can query it effectively.
...
- When `mos_scratchpad_summary()` shows edge density dropping below 1.0.
...
1. **Query with full visibility.** Always use `mos_scratchpad_query(limit=10000)` for maintenance. The default limit of 50 truncates the graph and causes incomplete maintenance.
...
5. **Target edge density >1.0 edges/node.** Below this threshold, community detection degrades. Check with `mos_scratchpad_summary()`.

6. **Verify structure with `mos_scratchpad_communities()`.** Healthy: 5–15 communities for a 500-node graph, largest <30% of total, multiple cross-community edges. Unhealthy: >50 (too sparse) or 1 (collapsed).

7. **Spot fragility with `mos_scratchpad_god_nodes()`.** Score >30 (degree >15) means the graph is too centralized. Split sub-hypotheses to distribute connectivity.
