...
Called by `run-review-round` during Pass A to produce one `reviewer-<i>.md`, where `<i>` is 1–5. May also be invoked directly when Reviewer main needs to add an extra reviewer instance to an existing round (e.g. when reviewer 3 was redundant and a 4th independent perspective is wanted before consolidation).
...
- A stance mix, drawn from `minions/roles/reviewer/personas/*.md`. Different stances within the same reviewer instance wherever possible — uniform stance collapses the dynamic mix.
...
3. **Choose stances.** Read `minions/roles/reviewer/personas/*.md`. Assign different stances to aspect subagents within this reviewer instance where possible.
4. **Spawn aspect subagents.** Each gets: current submission materials only, its aspect instructions, its stance excerpt, read-only / EACN-invisible / local-only boundary, output path, and `templates/aspect-note.md` as the required format.
5. **Merge aspect notes.** Reviewer main reads only the aspect notes for this instance and writes `reviewer-<i>.md` using `templates/reviewer-instance.md`.
