...
5. **Keep plotting scripts checked in.** `branches/writer/paper/figures/gen_fig_<name>.py` reads from a concrete data file under `branches/experimenter/experiments/` or `artifacts/exp-{id}/`. No hardcoded numbers. Re-run must reproduce byte-identical output modulo font rendering.
