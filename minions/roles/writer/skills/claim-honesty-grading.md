...
Flag any claim that would not survive a one-sentence challenge from a reviewer who has read the system implementation. Specific failure mode: Reviewer claimed "audits Atlas"; actual scope is "audits submission package" (Reviewer-Atlas overclaim, MinionsOS-Paper). Same defect appears as scope inflation in pillar names like "Pluggable Skill Nodes" when the actual scope also covers Workflow nodes.
...
- Renaming a pillar without checking that the new name doesn't *also* overclaim. "Pluggable Nodes" is broader than "Pluggable Skill Nodes" — verify the broader scope is real before adopting it.
