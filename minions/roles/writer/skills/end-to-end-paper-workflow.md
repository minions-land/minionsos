...
1. **Gather inputs first.** Read the project brief, `branches/<role>/` evidence files (primarily `branches/experimenter/` and `branches/coder/`), result tables/logs, existing figures, and any `template/` reference material. Do not write prose until the evidence inventory is clear.
...
6. **Integrate into `branches/writer/paper/`.** Treat `template/` as read-only, create or update the editable working copy under `branches/writer/paper/`, connect sections/figures/tables/bibliography, and compile.
