...
instances. Each skill node is spawned as an independent `expert-{slug}` with
...
workflow's repository into the skill node directory, point the manifest at its
...
```bash
# 1. Create the skill node directory
mkdir -p skill-nodes/{slug}/skills
...
See `skill-nodes/evoany/` for a complete worked example.
...
mcp_server:
  command: "node"
  args: ["skill-nodes/evoany/repo/dist/plugin/server.js"]
  env: {}
  tools:
    - evo_init
    - evo_step
...
```python
# Gru discovers available skill nodes
mos_list_skill_nodes()
# → [{"slug": "evoany", "name": "evoany", "has_mcp": true, ...}]

# Gru spawns an Expert with the skill node attached
mos_spawn_expert(
    project_port=37596,
...
- The skill node's MCP server in a per-instance config (not global `.mcp.json`)
- The skill node's `domain.md` appended to its system prompt
- The skill node's `skills/*.md` discoverable at wake-up
- Full EACN3 participation (messages, tasks, bids — not just Gru DM)

## Boundaries

- Skill node Experts write only to their own branch
...
- MinionsOS's own Scientific Discovery workflow is sovereign — skill nodes
...
mos_spawn_expert  → Expert process starts with skill node context
...
The skill-node architecture is designed to absorb the major categories of
scientific agent workflows. Below is how each category maps to a skill node.
...
| [EvoAny](https://github.com/DataLab-atom/EvoAny) | Already ships MCP server — mount directly (see `skill-nodes/evoany/`) |

**Key principle**: the external pipeline's internal agents are invisible to
EACN3. The skill-node Expert is the single point of contact. It drives the
...
wrap it as a single MCP surface. The skill-node Expert is the coordinator
...
- `skill-nodes/evoany/` — EvoAny evolutionary code optimization
