"""Performance comparison: Draft+compact vs long-running vs short-process.
...
class TestPerformanceComparison:
    ...
    def _simulate_dag_compact(self) -> dict:
        ...
        for _i, step in enumerate(SCENARIO):
            ...
            draft.mos_draft_append(nodes=[step["produces"]])

        # At step 10, query the DAG to verify knowledge retention
        summary = draft.mos_draft_summary()
        dead_ends = draft.mos_draft_query(node_type="dead_end")
    ...
    def test_paper_path_extraction(self):
        ...
        path = draft.mos_draft_path(target_node_id="D-001")
