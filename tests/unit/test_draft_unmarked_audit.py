...
class TestRoleUnmarkedRatioHelper:
    def test_ratio_counts_only_empty_tags(self):
        nodes = [
            {"id": "R1", "type": "result", "author_role": "coder", "evidence_tag": ""},
            {"id": "R2", "type": "result", "author_role": "coder", "evidence_tag": ""},
            {"id": "R3", "type": "result", "author_role": "coder", "evidence_tag": ""},
            {"id": "R4", "type": "result", "author_role": "coder", "evidence_tag": "exp/E1.md"},
            {"id": "R5", "type": "decision", "author_role": "coder", "evidence_tag": "exp/E2.md"},
        ]
        # 3 of 5 claim nodes are unmarked -> 0.6
        assert draft._role_unmarked_ratio(nodes, "coder") == 0.6
    ...
    def test_min_nodes_guard_returns_none(self):
        ...
        assert draft._role_unmarked_ratio(nodes, "coder") is None

    def test_non_claim_types_excluded(self):
        # plan / question / context are not claim-bearing; they must not move the ratio.
        nodes = [
            {"id": "R1", "type": "result", "author_role": "coder", "evidence_tag": "exp/E1.md"},
            {"id": "R2", "type": "result", "author_role": "coder", "evidence_tag": "exp/E2.md"},
            {"id": "R3", "type": "result", "author_role": "coder", "evidence_tag": "exp/E3.md"},
            {"id": "P1", "type": "plan", "author_role": "coder", "evidence_tag": ""},
            {"id": "Q1", "type": "question", "author_role": "coder", "evidence_tag": ""},
        ]
        # only the 3 result nodes count, all marked -> 0.0 (plans/questions ignored)
        assert draft._role_unmarked_ratio(nodes, "coder") == 0.0
...
class TestUnmarkedAudit:
    def test_aggregate_flags_above_threshold(self, _isolated_project: Path):
        _write_draft(
            _isolated_project,
            [
                # coder: 3/5 unmarked -> 0.6, flagged
                {"id": "R1", "type": "result", "author_role": "coder", "evidence_tag": ""},
                {"id": "R2", "type": "result", "author_role": "coder", "evidence_tag": ""},
                {"id": "R3", "type": "result", "author_role": "coder", "evidence_tag": ""},
                {"id": "R4", "type": "result", "author_role": "coder", "evidence_tag": "e"},
                {"id": "R5", "type": "decision", "author_role": "coder", "evidence_tag": "e"},
        ...
        assert out["per_role_unmarked"] == {"coder": 0.6, "ethics": 0.0, "expert": None}
        assert out["flagged_roles"] == ["coder"]
...
class TestAuthzBoundary:
    ...
    def test_peer_roles_denied(self):
        # Coder/Writer/Noter must not reach the audit tool server-side.
        for role in ("coder", "writer", "noter"):
            authz = resolve_server_authz(role, "main")
            # Noter/Coder/Writer have no mos_draft_* wildcard granting it, and the
            # explicit tool is not in their server authz list.
