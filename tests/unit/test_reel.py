...
def reel_env(monkeypatch, tmp_path):
    ...
    for role in ["expert", "expert-peer", "gru", "ethics", "expert-rl"]:
...
def _make_entry(
    role: str, session_id: str, tool_use_id: str, *, claude_jsonl: str = "", kind: str = "subagent"
) -> dict:
    return {
...
def test_validate_ref_component_accepts_normal():
    assert _validate_ref_component("expert", "role") == "expert"
...
def test_mos_reel_get_reads_claude_jsonl(reel_env, monkeypatch, tmp_path):
    _port, _tmp, branches_dir = reel_env
    monkeypatch.setenv("MINIONS_ROLE_NAME", "expert")

    jsonl_path = tmp_path / "s1.jsonl"
    _make_jsonl(jsonl_path, [{"turn": 1}, {"turn": 2}])

    entry = _make_entry("expert", "sess-1", "tid-001", claude_jsonl=str(jsonl_path))
    _write_index_entry(branches_dir, "expert", entry)

    result = mos_reel_get("expert/sess-1/tid-001")
    assert result["ref"] == "expert/sess-1/tid-001"
...
def test_mos_reel_get_no_jsonl_file(reel_env, monkeypatch):
    ...
    _port, _tmp, branches_dir = reel_env
    monkeypatch.setenv("MINIONS_ROLE_NAME", "expert")
    entry = _make_entry("expert", "sess-2", "tid-002", claude_jsonl="/nonexistent/path.jsonl")
    _write_index_entry(branches_dir, "expert", entry)
    result = mos_reel_get("expert/sess-2/tid-002")
    assert result["lines"] == []
...
def test_mos_reel_get_permission_denied(reel_env, monkeypatch, tmp_path):
    ...
    _port, _tmp, branches_dir = reel_env
    monkeypatch.setenv("MINIONS_ROLE_NAME", "expert")
    entry = _make_entry("expert-peer", "sess-3", "tid-003")
    _write_index_entry(branches_dir, "expert-peer", entry)
    with pytest.raises(PermissionError, match="cannot read reel for role 'expert-peer'"):
        mos_reel_get("expert-peer/sess-3/tid-003")
...
def test_mos_reel_get_ethics_cross_read(reel_env, monkeypatch, tmp_path):
    ...
    jsonl_path = tmp_path / "expert_session.jsonl"
    _make_jsonl(jsonl_path, [{"data": "secret"}])
    entry = _make_entry("expert", "sess-4", "tid-004", claude_jsonl=str(jsonl_path))
    _write_index_entry(branches_dir, "expert", entry)
    result = mos_reel_get("expert/sess-4/tid-004")
    assert result["role"] == "expert"
...
def test_mos_reel_get_gru_cross_read(reel_env, monkeypatch, tmp_path):
    ...
    entry = _make_entry("expert", "sess-5", "tid-005")
    _write_index_entry(branches_dir, "expert", entry)
    result = mos_reel_get("expert/sess-5/tid-005")
    assert result["role"] == "expert"
...
def test_mos_reel_window_returns_centred_slice(reel_env, monkeypatch):
    ...
    result = mos_reel_window("expert/sess-w/003", span=2)
    assert len(result) == 5
    refs = [e["ref"] for e in result]
    assert "expert/sess-w/003" in refs
    assert refs[0] == "expert/sess-w/001"
    assert refs[-1] == "expert/sess-w/005"


def test_mos_reel_window_ref_not_found(reel_env, monkeypatch):
    _port, _tmp, _branches_dir = reel_env
    monkeypatch.setenv("MINIONS_ROLE_NAME", "expert")
    with pytest.raises(ReelError, match="not found in reel index"):
        mos_reel_window("expert/sess-missing/tid-xxx")
...
def test_mos_reel_backfill_adds_draft_ref(reel_env, monkeypatch):
    ...
    entry = _make_entry("expert", "sess-b", "tid-b01")
    _write_index_entry(branches_dir, "expert", entry)

    result = mos_reel_backfill_draft_ref("expert/sess-b/tid-b01", "H-007")
    assert result["status"] == "ok"

    # Re-read the index and confirm the ref is present
    from minions.tools.reel import _read_index

    index_path = branches_dir / "expert" / "reel-index.jsonl"
    entries = _read_index(index_path)
    target = next(e for e in entries if e["ref"] == "expert/sess-b/tid-b01")
...
def test_mos_reel_backfill_idempotent(reel_env, monkeypatch):
    ...
    entry = _make_entry("expert", "sess-b2", "tid-b02")
    _write_index_entry(branches_dir, "expert", entry)

    mos_reel_backfill_draft_ref("expert/sess-b2/tid-b02", "Q-003")
    mos_reel_backfill_draft_ref("expert/sess-b2/tid-b02", "Q-003")
    ...
    index_path = branches_dir / "expert" / "reel-index.jsonl"
    entries = _read_index(index_path)
    target = next(e for e in entries if e["ref"] == "expert/sess-b2/tid-b02")
