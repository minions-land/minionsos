...
from minions.tools import draft, reset
...
def test_pending_plan_flag_survives_dag_round_trip(project_port: int, tmp_path: Path) -> None:
    """metadata.pending_plan must persist to draft.json and reload intact."""
    draft.mos_draft_append(
        nodes=[
            {
                "type": "question",
