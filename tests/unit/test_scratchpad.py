"""Unit tests for minions.tools.scratchpad."""
...
def _isolated_project(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    scratchpad_dir = tmp_path / f"project_{port}" / "branches" / "shared" / "scratchpad"
    scratchpad_dir.mkdir(parents=True)
    return scratchpad_dir


def _write_legacy_scratchpad(scratchpad_dir: Path, nodes: list[dict[str, object]]) -> None:
    payload = {
        "project_port": 9999,
    ...
    (scratchpad_dir / "scratchpad.json").write_text(json.dumps(payload), encoding="utf-8")


class TestMosScratchpadAppend:
    def test_append_single_node(self):
        result = scratchpad.mos_scratchpad_append(nodes=[{"type": "hypothesis", "text": "Test hypothesis"}])
        assert result["created_node_ids"] == ["H-001"]
        assert result["created_edge_count"] == 0

    def test_append_auto_increments_ids(self):
        scratchpad.mos_scratchpad_append(nodes=[{"type": "hypothesis", "text": "First"}])
        result = scratchpad.mos_scratchpad_append(nodes=[{"type": "hypothesis", "text": "Second"}])
        assert result["created_node_ids"] == ["H-002"]

    def test_append_multiple_types(self):
        result = scratchpad.mos_scratchpad_append(
            nodes=[
    ...
    def test_append_custom_type_accepted(self):
        ...
        result = scratchpad.mos_scratchpad_append(nodes=[{"type": "observation", "text": "X"}])
    ...
    def test_append_stores_provenance_and_confidence(self):
        ...
        node = scratchpad.mos_scratchpad_query(node_type="hypothesis")["nodes"][0]
        assert node["provenance"] == "inferred"
        assert node["confidence"] == 0.42

    def test_append_defaults_provenance_and_confidence(self):
        scratchpad.mos_scratchpad_append(nodes=[{"type": "hypothesis", "text": "Defaulted"}])
        node = scratchpad.mos_scratchpad_query(node_type="hypothesis")["nodes"][0]
    ...
    def test_append_accepts_custom_provenance(self, caplog: pytest.LogCaptureFixture):
        with caplog.at_level(logging.INFO, logger=scratchpad.logger.name):
            scratchpad.mos_scratchpad_append(
                nodes=[
                    {
                        "type": "hypothesis",
    ...
    def test_journal_written(self, _isolated_project: Path):
        scratchpad.mos_scratchpad_append(nodes=[{"type": "question", "text": "Why?"}])
...
class TestMosScratchpadQuery:
    def test_query_empty_dag(self):
        result = scratchpad.mos_scratchpad_query()
        assert result["nodes"] == []
    ...
    def test_query_by_type(self):
        ...
        result = scratchpad.mos_scratchpad_query(node_type="hypothesis")
    ...
    def test_query_by_status(self):
        ...
        result = scratchpad.mos_scratchpad_query(support_status="verified")
        assert len(result["nodes"]) == 1
    ...
    def test_legacy_dag_without_provenance_confidence_still_works(
        ...
        query = scratchpad.mos_scratchpad_query(related_to="H-001")
...
class TestMosScratchpadAnnotate:
    def test_annotate_status(self):
        scratchpad.mos_scratchpad_append(nodes=[{"type": "hypothesis", "text": "H1"}])
    ...
    def test_annotate_nonexistent_raises(self):
        with pytest.raises(ValueError, match="Node not found"):
            scratchpad.mos_scratchpad_annotate(node_id="X-999", support_status="verified")


class TestMosScratchpadPath:
    def test_path_simple_chain(self):
        ...
        result = scratchpad.mos_scratchpad_path(target_node_id="E-001")
        assert len(result["path_nodes"]) == 3
        assert result["path_nodes"][0]["id"] == "Q-001"
        assert result["path_nodes"][-1]["id"] == "E-001"


class TestMosScratchpadSummary:
    def test_summary_empty(self):
        result = scratchpad.mos_scratchpad_summary()
