"""Unit tests for the Phase 2 Wiki Layer 2 surface."""
...
from minions.tools import publish, wiki
...
def test_wiki_ingest_creates_page_index_and_log(
    ...
    page = project["shared"] / "wiki" / "sources" / "coder-transformer-run.md"
    assert page.exists()
    ...
    assert "wiki_path: wiki/sources/coder-transformer-run.md" in index

    log_path = project["shared"] / "wiki" / "log.md"
    ...
    assert result["wiki_path"] == "wiki/sources/coder-transformer-run.md"
...
def test_wiki_ingest_is_idempotent_for_index_entries(
    ...
    wiki.mos_wiki_ingest(str(source), "coder", "same-slug", title="First", port=project["port"])
...
def test_wiki_ingest_missing_source_raises(project: dict[str, Any]) -> None:
    ...
    with pytest.raises(wiki.WikiError, match="src_path does not exist"):
        wiki.mos_wiki_ingest(str(missing), "coder", "missing", port=project["port"])


def test_wiki_query_returns_matching_index_entry(project: dict[str, Any]) -> None:
    index = project["shared"] / "wiki" / "index.md"
    index.write_text(
        "\n".join(
            [
                "# Wiki Index",
    ...
    result = wiki.mos_wiki_query("transformer", port=project["port"])
...
def test_wiki_hot_get_reads_existing_file(project: dict[str, Any]) -> None:
    hot = project["shared"] / "wiki" / "hot.md"
...
def test_publish_policy_rejects_ethics_wiki() -> None:
    with pytest.raises(ProjectError, match="may not publish under branches/shared/wiki"):
        publish._validate_dst("ethics", "wiki/audit.md")


def test_publish_policy_allows_noter_wiki() -> None:
    assert publish._validate_dst("noter", "wiki/sources/source.md") == Path(
